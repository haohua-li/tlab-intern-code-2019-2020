...
bool AllStates::CheckSynchError(int index, int state, char *&synchString0,
  ...
  if (diff > 0) {
    ...
    if (state == NULL_STATE && ((index >= dataSize - 1) || (!isMulti))) {
      mexErrMsgTxt("\nError: Could not synchronize, cannot use this data. \nAllStates::GetStateDistsMulti\n\nNote: For some reason this program cannot synchronize to astate at any point in the data.  See 'ReadMe' for details.\n\n"); 
    }
...
//////////////////////////////////////////////////////////////////////////
int AllStates::SynchToStatesMulti(int &index, int &lineIndex, int state, int maxLength, 
                                    ofstream *outData, char *data, int dataLength, char *&synchString) {
