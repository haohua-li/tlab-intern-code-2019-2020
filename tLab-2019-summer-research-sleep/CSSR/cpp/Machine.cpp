...
//////////////////////////////////////////////////////////////////////////
void Machine::PrintDot(char output[], char alpha[]) {
  //create file streams
  ofstream outData(output, ios::out);

  //open output file, if unsuccessful exitt
  if (!outData) {
...
///////////////////////////////////////////////////////////////////////////
// Function: Machine::PrintDot2
// Purpose: prints out all the machine to a .txt file
// In Params: the name of the input file used for program, the alphabet
// Out Params: none
// In/Out Params: none
// Pre- Cond: all values in array of states have been set
// Post-Cond: output .txt file exists with machine
/////////////////////////////////////////////////////////////////////////
void Machine::PrintDot2(char output[], char alpha[]) {
  //create file streams
  ofstream outData(output, ios::out);

  //open output file, if unsuccessful exitt
  if (!outData) {
    mexErrMsgTxt(" the .txt output file cannot be opened \n"); 
    //otherwise output data
  }
  else {
    int size = m_allstates->getArraySize();
    ...
    for (int i = 0; i < size; i++) {
      ...
      for (int k = 0; k < distSize; k++) {
        ...
        if (nextState != NULL_STATE) {
          outData << i << " " << nextState;
          outData << " " << alpha[k]
          << " " << setiosflags(ios::left) << setw(7) << setprecision(8) << dist[k] << endl;
        }
      }
    }
    outData << endl;
